...
TEST_CASE("disk scheduler: single-shot vs batched throughput", "[perf]") {
  ...
  // Single-shot path.
  const auto single_start = clock_t::now();
  std::vector<std::future<bool>> single_futures;
  single_futures.reserve(kNumRequests);
  for (int i = 0; i < kNumRequests; ++i) {
    single_futures.push_back(single_scheduler.Schedule(
        im110::DiskRequest::Kind::Write, i, single_buffers[i].data()));
  }
  for (auto& f : single_futures) {
    REQUIRE(f.get());
  }
  const auto single_elapsed = clock_t::now() - single_start;

  // Batched path.
  const auto batched_start = clock_t::now();
  ...
  const auto batched_elapsed = clock_t::now() - batched_start;
  ...
  std::printf("[perf] single-shot: %lld ms, batched: %lld ms (n=%d, latency=%lldms)\n",
              ...
              static_cast<long long>(to_ms(batched_elapsed)),
              kNumRequests,
